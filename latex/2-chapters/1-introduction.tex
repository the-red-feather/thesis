...
(add a sentence about dataflow modelling)
...
Many new \ac{geo-vpl}s are continuously being developed, as the existing ones were deemed inadequate. 
...
% all of this is to say: web-based geocomputation is a valuable topic 
...
\begin{enumerate}[-]
  \item Accessibility: no installment or configuration needed, cross-platform by default
  \item Distribution: web applications can quickly be updated, shared, licensed. 
\end{enumerate}
...
\begin{note}
presupposition: combining a geo-vpl and bbg is useful.
\end{note}
