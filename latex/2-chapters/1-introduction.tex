...
% state of the art regarding this issue
...
\begin{itemize}
  ...
  \item Not many implementations using WebAssembly. 
  \item Not many accessible, open tools
\end{itemize}
...
Due to the need for a first step towards wasm-based geo-computation, the goal of this thesis is to develop a method for geo-computation in a browser-based front-end. 
the browser front-end is used as a proxy for containerization. By making a process run in a front-end web environment, it is safe to assume can be run on the cloud. Additionally, by 
