...
With both developments used in conjunction, applications can be created which use no active servers at all: Static GIS web applications.
...
The overarching goal of this study is to allow \ac{GIS} practitioners without a background in software development, to access the full potential of core transformation and analysis capabilities found in native \ac{GIS} libraries.

The study attempts to meet this goal by designing, implementing, and accessing a prototype based on the fields of visual programming, and static web applications. 
...
The design of this prototype is as follows: 
A VPL is used to present the functionalities of \ac{GIS} libraries from within an application, and in a composable manner. Additionally, it is used to connect these libraries to various \ac{GUI} features. 
...
Finally, as this prototypes aspires to qualify as a \ac{VPL} specialized for \ac{GIS}, both scalability to handle sizable datasets, and rich \ac{GUI} support (3D viewers, file inputs, sliders, text boxes, graphs), are primary design considerations and assessment criteria. 

While VPLs and even Web-based VPLs have been studied before, the focus on direct utilization of native \ac{GIS} libraries, combined with bringing those in direct contact with various \ac{GUI} elements, and all without resorting to active web servers, is a combination unique to this study, at least to the best of the authors knowledge at this point in time. 
...
The compilation of native \ac{GIS} libraries to Webassembly has several caveats, and achieving scalability with a VPL requires novel, untested methods. 
Therefore, seeking and analyzing solutions to these challenges is required to making this prototype meet the specifications of the design mentioned. 
...
\begin{itemize}[ ]
  \item \textit{Does a web-based, GUI-rich VPL improve the \textbf{accessibility} and \textbf{composability} of native GIS libraries, compared to similar VPLs?}
\end{itemize}

\subsection*{Supporting Questions}

% \begin{note}
%   QUESTION: What makes it Spatial?!?! ANSWER: scalability. dataflow VPL
% \end{note}

\begin{itemize}[-]
\item How can native GIS libraries be compiled, loaded, and utilized within a browser-based VPL?

\item What measures are taken to make this VPL scalable to large geo-datasets, and how effective are these measures?
%  (2. Are the functional properties of a dataflow-VPL uphold by this solution?)

\item What GUI features are required to facilitate this VPL, and to what extend does the web platform aid or hurt these features?

\item to what extend does this solution "take the best of both software applications and libraries", as described by (SOURCE)?
...
\begin{note}
TODO: What is the assessment?? 

Methodology: assessment lists and criteria are created. 

When we do compare this VPL, we compare it to similar VPLs.
-> similar in terms of computer graphics-related VPLs. 
% How does this solution compare to similar VPLs? 
