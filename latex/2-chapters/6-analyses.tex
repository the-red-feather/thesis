...
(rust-> wasm is powerful and feature-rich. it functions like a normal compiler. compilation was done easely. If a library cannot be compiled, the compiler statements are clear enough to identify which library causes the in)
(C++ -> wasm requires a lot of trivia knowledge. it cannot be done often, or easely. Also behind on documentation compared to Rust).
...
- dataflow programming aspects allow for all dataflow nicieties.
- However, the openness of the plugin system inhibits the consistency of the language.
  - libraries can create functions with many side effects, or they can use inconsistent input and output datatypes.
  - This ultimately leads to confusion for the end-user.
