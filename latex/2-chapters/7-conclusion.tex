...
Two 'layers' of \ac{GUI} features had to be created:
...
The implementation can indeed be used make web applications composable, evident from the demo application presented in \refsec{sec:testing:demo}.
...
The conclusion of this component of the study is a dillema between C / C++ and Rust.
Based on the experiments and analysis in \refsec{sec:testing:compilation}, it can be concluded that most contemporary, C/C++-based geocomputation libraries cannot be sufficiently compiled in a manner which would have allowed them to be loaded within the web-VPL.    
...
The Rust programming language offers a performance and level of control similar to C++, but its \m{wasm-bindgen} toolkit offers better wasm-library support. 
...
However, all measures have certain shortcomings:
...
\begin{enumerate}
  ...
  \subitem \textbf{Shortcoming}: The \ac{wasm} binaries must be wrapped using javascript, because \ac{wasm} does not support interface types yet \citep{wagner_interface_2022}. This means that currently, a backend would require a javascript runtime to use these libraries.
  ...
  \subitem \textbf{Shortcoming}: The no-boilerplate plugin system led to complication for incorporating C / C++ libraries.
  ...
  \subitem \textbf{Shortcoming}: The freedom of the no-boilerplate plugin system, combined with javascript limitations, made it so the functional programming properties of this DataFlow VPL could not be strictly enforced. 
...
The 'no-boilerplate' plugin system cannot be used with C / C++ \ac{GIS} libraries, currently, but this might be fixed when the emscripten compiler adopts the Interface Types proposal. 
