...
Since giving a full overview of all aspects of these bodies of work is too extensive, only key elements within these bodies will be mentioned and elaborated. 
...
\ac{wasm} is a binary instruction format for a conceptual, stack-based virtual machine 
...
Contrary to its name, WebAssembly has no specific link to the web or assembly language. 
It was, however, designed for the web. 
\ac{wasm} is officially dubbed the fourth type of programming language supported by all major web browsers, next to HTML, CSS, and JavaScript (Source: mozilla post).
...
It is important to note that these features offer a reverse workflow compared to the now popular Electron based applications described in \refsec{sec:background-web-rich}.
...
The WebAssembly format has several other use cases (SOURCE: https://blog.scottlogic.com/2022/06/20/state-of-wasm-2022.html).


\begin{note}
  TODO: convert all the dumb client / server writings into a chapter which may live here  
\end{note}
...
A \ac{vpl} has the potential to make automation available to a very large audience, and this is exactly its purpose. 
