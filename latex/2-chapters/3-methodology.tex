...
The methodology of this study can be characterized as practical as opposed to theoretical, wholistic instead of specific, and iterative compared to linear. The prior works on browser-based geocomputation and geo-vpls indicate that a strong theoretical framework for a \ac{geo-web-vpl} is in place. 
...
However, the absence of a geo-web-vpl indicates some sort of hinder, preventing this type of application to be realized. Either geo-VPLs were not able to be properly used in a browser, or web-based VPLs unable to support geocomputation functionalities. 
...
Each one of these four factors is visited, this prototype is used as an experiment to discover the extend of the possible hindrances. 
Per factor, we explore different options, document considerations, and test to what extend the \ac{geo-web-vpl} fails or succeeds to provide for this aspect. 
...
A 3D VPL is defined as a vpl meant for 3D data processing. 

The method to answer this question is defined as follows. 
The above question of \mySubRQOneTitle is subdivided into the following follow-up questions:
\begin{enumerate}
  \item \emph{What are the requirements of a 3D VPL interface?}.
  \item \emph{What can be defined as core browser features?}.
  \item \emph{Per requirement, to what extend these core browser features be used to implement this requirement?}.
  \item \emph{Per implemented requirement, to what extend does a web implementation differ from existing, popular 3D VPLs?}.
\end{enumerate}

The first and second question will be answered subsequently. The third question is answered by \refsec{sec:app}, and the answer to the fourth question can be found in the Conclusion, as it doubles as the answer to the entire question of \mySubRQOneTitle.

\subsection{3D VPL requirements}

Based on the vpl research (SOURCE), any visual programming language must at the very least contain the following aspects: 
\begin{enumerate}[-]
  \item a visual language, represented as a flowchart
  \item an interface to edit this visual language 
  \item a way to load input data 
  \item a way to save output data
\end{enumerate}
...
\begin{enumerate}[-]
  ...
  \item multiple ways to view output data (text displays)
\end{enumerate}

\subsection{core browser features}
This study defines "Core browser features" as the set of default features implemented by the three largest browser vendors. 
These browser vendors are chosen on market share:
\begin{note}
  (show pie chart of usage statistics)
\end{note}
This indicates the following browsers as 'major':
\begin{enumerate}[-]
  \item Chrome \& Chromium based browsers
  \item Firefox.
  \item Safari
\end{enumerate}
...
In this section, we wish to discover ... encompassed by the research question : \mySubRQTwo
For this, the following plan was used: 

how to take existing libraries, and lift them up to the web?

(Me in the future: read the 'old' stuff, might have some fun stuff, but first define what you want.)

\section{\mySubRQThreeTitle} 
The third component of the methodology involved ...
In this section, we wish to discover ... encompassed by the research question : \mySubRQThree
For this, the following plan was used: 

- How to take advantage of the medium? 

how to take the uplifted libraries, and load them into the 3D VPL? 

\section{\mySubRQFourTitle} 
The final component of the methodology involved... 
...
- "Is a 3D vpl" + some geolibraries magically a geo-web-vpl? 
...
This section of the methodology will be about

implementing the foundations of the prototype. 
