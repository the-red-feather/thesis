...
\begin{note}
  A framework was desired akin to what a desktop application might expect to configure these aspects.
However, while the browser-based software landscape did provide such a framework
\end{note}
...
This way, the application could be run headless either the browser or on a server, using a local javascript runtime like Deno \citep{contributors_deno_2022}.
However, the Geofront to javascript compiler could not be maintained given the other requirements of the environment, such as complex \ac{GUI} nodes.
