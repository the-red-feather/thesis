...
This section offers a background on frontend web applications and developments in frontend web software development and frontend web technologies.

\subsection{Distributed systems}

In software development of distributed systems, the following dichotomies exist: 
\begin{enumerate}[-]
...
A soft distinction is also made between websites, and web applications. 
...
Lastly, the nuances between a web application and a native application must be specified. 
...
If a client runs without the active corresponding server it relies upon, we can say that the client functions, but the entire \emph{application} does not function. 
...
An program is considered web-based or browser-based if a browser is required to run it. 
...
A native program can also be a client, as there is nothing preventing a native program of making the exact same web request as a web application. 
...
As soon as a web app is be found, it can theoretically be used.
...
The disadvantage is that normally, web applications can only be written in one programming language: JavaScript. 
...
This layer of indirection makes web applications in general unfavorable for high performance computation. 

\subsection{Trends}

With all these dichotomies in mind, 

The web started out with centralized client-server models. 
a client request a web page, a server composes and serves this html, and the browser takes care of formatting and rendering this response to the screen of the user.  
As this model was highly restrictive to more interactive or demanding applications, 
...
This trend has not stopped. Since 2012, a trend of rich web-clients can be recognized \cite{hamilton_client-side_2014, panidi_hybrid_2015, kulawiak_analysis_2019}.
...
WebAssembly leads to a complete re-examination of established client vs server, and web vs native dichotomies.
