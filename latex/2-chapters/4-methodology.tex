...
The overall methodology is to first develop a base VPL (\refsec{sec:method-base-vpl}), followed up by developing a binding system for this VPL (\refsec{sec:method-bindings}). 
The final step is to set up and execute a series of tests (\refsec{sec:method-bindings}), to analyze the extend to which these implementations solve the various challenges raised by \refchap{chap:intro}.
...
These requirements lead to the following design.
A plugin:
\begin{enumerate}[-]
...
The plugin system will utilize the infrastructure of regular javascript libraries as much as possible. 

Additionally, this setup adresses the portability requirement by making these libraries interoperable with unaffiliated projects on both the frontend and backend (see \reffig{fig:plugin-model}).
...
\begin{enumerate}
  ...
  \item Optional: Include metadata about how to use this plugin.
  \item Compile this library with a compatible compiler: Emscripten for C++ (Source), 'wasm-pack' combined with 'wasm-bindgen' for rust (Source).
...
\end{enumerate}
Several steps in this workflow require elaboration, which will be given in the subsequent sections: 
...
\begin{figure}
  ...
  \graphicspath{ {../../assets/diagrams/} }
  \includegraphics[width=\linewidth]{Model Proposal.png}
  \caption{TODO: Proposed Plugin model}
  \label{fig:plugin-model}
\end{figure}


The central idea for this model is to either interpret the required information from the wasm binary and related files, or, if that is impossible, add the required information in the wasm wrapper library itself as meta data.
...
\newpage
\section{Tests}
The purpose of this final step of the methodology is to gather the results and data needed for properly answering the second, third fourth sub-research questions. 

\subsection{Compilation Tests}

The first test focusses on making a measurable comparison between compilation methods, where the second experiment focusses on compilation in a practical, realistic scenario. 
