...
This makes the 'canvas API' (Source) an ideal fit to this component.
...
For C++, the Emscripten compiler can be used to compile to WebAssembly (Source). 
A minimum example of what a 'emscripten-ready-library' looks like can be seen in \reffig{fig:minimum-cpp-wasm}.

For Rust, the 'wasm-pack' and 'wasm-bindgen' toolkits enable \ac{wasm} compilation (Source).
...
The first set of tests focusses on making a comparison between compilation methods.

This is a qualitative comparison between compiling a full-scale library written in Rust, to a full library written in C++. 
