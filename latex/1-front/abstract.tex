...
In the field of \ac{GIS}, transformation and analysis tools often take the shape of software libraries, written in system level programming languages. 
A number of these libraries, like PROJ, GDAL, and CGAL, play a foundational role in almost all \ac{GIS} software applications.
However, for end users, these libraries are only usable to a limited extend.
When these libraries are distributed to other languages and applications, 
features and performance get 'lost in translation'.
Additionally, the applications end-users are left with are often not composable to the same degree as the libraries. 
...
As such, the goal of this study is to make core \ac{GIS} libraries more directly available to end-users.
...
WebAssembly is used so that \ac{GIS} libraries written in native languages only have to write one generic binding, which then can be used in any language, including this prototypical, web based visual language, thanks to a 'no boilerplate' plugin system. 
% WebAssembly is also used to run  resorting to active backend web services. 
Finally, as this prototype is intended for \ac{GIS} usage, both scalability to handle sizable datasets, and a rich \ac{GUI} (3D viewers, file inputs, sliders), are primary design considerations and assessment criteria.
% be used without
% Using this, ...
% indeed 

The results of the study are carefully positive. 
This specific web-based VPL appears to be a feasible method for providing direct access to some native \ac{GIS} libraries, and does offer a unique set of features not found in comparable visual languages. 
