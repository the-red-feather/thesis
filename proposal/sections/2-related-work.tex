...
However, we are required to examine these studies more closely as well as introduce others, in order to further define our methodology. This study's methodology will draw from prior studies on three topics: The topic of the Geoweb, the topic of browser-based geoprocessing, and prior studies on WebAssembly.
...
As a side note, client-side geoprocessing is not to be confused with native geoprocessing clients, which would include applications like QGIS \cite{noauthor_welcome_nodate} or ArcGIS \cite{esri_account_nodate}. 
...
What this means for the methodology, is that a significant portion of this study's attention will have to go to experimenting with different ways of compiling to WebAssembly, while making sure it can be used in a realistic scenario.
If it turns out that the use-case app can only be used by experienced end-users which take special \ac{wasm} considerations in mind, a big reason of using \ac{wasm}, namely its accessibility, would be lost.  